...
public static class ProfileEndpoints
{
    ...
    private static async Task<IResult> Get(PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        ...
        if (profile is null)
        {
            return ApiProblems.Create(http, 404, "Profile not found");
        }
    ...
    private static async Task<IResult> Create(ProfileRequest request, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        ...
        if (await db.Profiles.IgnoreQueryFilters().AnyAsync(ct))
        {
            return ApiProblems.Create(http, 409, "Profile already exists");
        }
        ...
        var profile = new Profile
        {
            ...
            ShortSummary = Trim(request.ShortSummary),
            Location = Trim(request.Location),
            Email = Trim(request.Email),
            Availability = Trim(request.Availability),
            CurrentFocus = Trim(request.CurrentFocus),
    ...
    private static async Task<IResult> Patch(JsonElement document, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        ...
        profile.ShortSummary = Trim(request.ShortSummary);
        profile.Location = Trim(request.Location);
        profile.Email = Trim(request.Email);
        profile.Availability = Trim(request.Availability);
        profile.CurrentFocus = Trim(request.CurrentFocus);
        ...
        profile.Version++;
        profile.PublicUpdatedAt = DateTimeOffset.UtcNow;
        try
        {
            await db.SaveChangesAsync(ct);
        }
