...
public static class SkillEndpoints
{
    public static void Map(RouteGroupBuilder admin)
    {
        ...
        group.MapGet("", List); group.MapPost("", Create); group.MapGet("/{id:guid}", Get);
        group.MapMethods("/{id:guid}", ["PATCH"], Patch).Accepts<JsonElement>("application/merge-patch+json");
        group.MapDelete("/{id:guid}", Delete); group.MapPost("/{id:guid}/restore", Restore);
    }
    ...
    private static async Task<IResult> Get(Guid id, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        ...
        HttpConcurrency.Set(http.Response, value.Version); return Results.Ok(value.ToResponse());
    }
    ...
    {
        var errors = InputValidation.Skill(request); if (errors.Count > 0) return ApiProblems.Validation(http, errors);
        if (!await db.SkillCategories.AnyAsync(x => x.Id == request.CategoryId, ct)) return ApiProblems.Validation(http, new() { ["categoryId"] = ["Must reference an active category."] });
        var normalized = InputValidation.Normalize(request.Name!);
        if (await db.Skills.AnyAsync(x => x.CategoryId == request.CategoryId && x.NormalizedName == normalized, ct)) return Conflict(http);
        var value = new Skill { Name = request.Name!.Trim(), NormalizedName = normalized, CategoryId = request.CategoryId!.Value };
        db.Skills.Add(value); await db.SaveChangesAsync(ct); HttpConcurrency.Set(http.Response, value.Version);
    ...
    private static async Task<IResult> Patch(Guid id, JsonElement document, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        ...
        if (!await db.SkillCategories.AnyAsync(x => x.Id == request.CategoryId, ct)) return ApiProblems.Validation(http, new() { ["categoryId"] = ["Must reference an active category."] });
        var normalized = InputValidation.Normalize(request.Name!);
        if (await db.Skills.AnyAsync(x => x.Id != id && x.CategoryId == request.CategoryId && x.NormalizedName == normalized, ct)) return Conflict(http);
        value.Name = request.Name!.Trim(); value.NormalizedName = normalized; value.CategoryId = request.CategoryId!.Value; value.Version++; value.PublicUpdatedAt = DateTimeOffset.UtcNow;
    ...
    {
        var value = await db.Skills.IgnoreQueryFilters().SingleOrDefaultAsync(x => x.Id == id, ct); if (value is null) return NotFound(http);
        ...
        if (value.DeletedAt is not null) return Results.NoContent(); var precondition = HttpConcurrency.Validate(http, value); if (precondition is not null) return precondition;
    ...
    {
        var value = await db.Skills.IgnoreQueryFilters().SingleOrDefaultAsync(x => x.Id == id, ct); if (value is null || value.DeletedAt is null) return NotFound(http);
        var precondition = HttpConcurrency.Validate(http, value); if (precondition is not null) return precondition;
    ...
    private static IResult Conflict(HttpContext http) => ApiProblems.Create(http, 409, "Skill already exists in this category");
    private static IResult NotFound(HttpContext http) => ApiProblems.Create(http, 404, "Skill not found");
}
