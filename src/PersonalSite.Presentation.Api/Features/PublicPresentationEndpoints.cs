...
{
    public static void Map(IEndpointRouteBuilder endpoints) => endpoints.MapGet("/api/v1/presentation", Get).AllowAnonymous();
    ...
    private static async Task<IResult> Get(PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        ...
        if (profile is null) return ApiProblems.Create(http, 404, "Presentation not found", "The profile has not been initialized.");
        ...
        timestamps.AddRange(experiences.Select(x => x.PublicUpdatedAt)); timestamps.AddRange(projects.Select(x => x.PublicUpdatedAt));
        timestamps.AddRange(categories.Select(x => x.PublicUpdatedAt)); timestamps.AddRange(categories.SelectMany(x => x.Skills).Select(x => x.PublicUpdatedAt));
        ...
        http.Response.Headers.ETag = etag; http.Response.Headers.CacheControl = "public,max-age=60,must-revalidate";
        if (http.Request.Headers.IfNoneMatch.Any(x => string.Equals(x, etag, StringComparison.Ordinal))) return Results.StatusCode(StatusCodes.Status304NotModified);
