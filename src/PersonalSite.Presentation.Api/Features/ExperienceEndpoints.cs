...
public static class ExperienceEndpoints
{
    ...
    {
        var group = admin.MapGroup("/experiences"); group.MapGet("", List); group.MapPost("", Create); group.MapGet("/{id:guid}", Get);
    ...
    private static IQueryable<Experience> Expanded(PresentationDbContext db) => db.Experiences.Include(x => x.Highlights).Include(x => x.Technologies);
    ...
    {
        var value = await Expanded(db).SingleOrDefaultAsync(x => x.Id == id, ct); if (value is null) return NotFound(http); HttpConcurrency.Set(http.Response, value.Version); return Results.Ok(value.ToResponse());
    }
    private static async Task<IResult> Create(ExperienceRequest request, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        ...
        var value = Build(request); db.Experiences.Add(value); await db.SaveChangesAsync(ct); HttpConcurrency.Set(http.Response, value.Version);
    ...
    {
        var value = await db.Experiences.IgnoreQueryFilters().SingleOrDefaultAsync(x => x.Id == id, ct); if (value is null) return NotFound(http);
        if (!http.Request.Headers.ContainsKey("If-Match")) return ApiProblems.Create(http, 428, "Precondition Required", "If-Match is required."); if (value.DeletedAt is not null) return Results.NoContent();
        var precondition = HttpConcurrency.Validate(http, value); if (precondition is not null) return precondition; value.DeletedAt = DateTimeOffset.UtcNow; value.Version++; value.PublicUpdatedAt = DateTimeOffset.UtcNow; await db.SaveChangesAsync(ct); HttpConcurrency.Set(http.Response, value.Version); return Results.NoContent();
    }
    private static async Task<IResult> Restore(Guid id, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        var value = await db.Experiences.IgnoreQueryFilters().Include(x => x.Technologies).SingleOrDefaultAsync(x => x.Id == id, ct); if (value is null || value.DeletedAt is null) return NotFound(http);
        var precondition = HttpConcurrency.Validate(http, value); if (precondition is not null) return precondition;
        if (!await TechnologyIdsAreActive(db, value.Technologies.Select(x => x.TechnologyId).ToList(), ct)) return ApiProblems.Create(http, 409, "Experience references a deleted technology");
        value.DeletedAt = null; value.Version++; value.PublicUpdatedAt = DateTimeOffset.UtcNow; await db.SaveChangesAsync(ct); HttpConcurrency.Set(http.Response, value.Version); return Results.NoContent();
    }
    private static Experience Build(ExperienceRequest request) => new() { Company = request.Company!.Trim(), Role = request.Role!.Trim(), Location = request.Location?.Trim(), StartDate = request.StartDate!.Value, EndDate = request.EndDate, Summary = request.Summary!.Trim(), Highlights = request.Highlights!.Select(x => new ExperienceHighlight { Text = x.Trim() }).ToList(), Technologies = request.TechnologyIds!.Select(x => new ExperienceTechnology { TechnologyId = x }).ToList() };
    internal static async Task<bool> TechnologyIdsAreActive(PresentationDbContext db, List<Guid> ids, CancellationToken ct) => await db.Technologies.CountAsync(x => ids.Contains(x.Id), ct) == ids.Count;
    ...
    private static IResult NotFound(HttpContext http) => ApiProblems.Create(http, 404, "Experience not found");
}
