...
public static class NamedResourceEndpoints
{
    ...
    private static void MapCategories(RouteGroupBuilder group)
    {
        ...
        group.MapMethods("/{id:guid}", ["PATCH"], PatchCategory).Accepts<JsonElement>("application/merge-patch+json");
    ...
    {
        group.MapGet("", ListTechnologies);
        group.MapPost("", CreateTechnology);
        group.MapGet("/{id:guid}", GetTechnology);
        group.MapMethods("/{id:guid}", ["PATCH"], PatchTechnology).Accepts<JsonElement>("application/merge-patch+json");
        group.MapDelete("/{id:guid}", DeleteTechnology);
        group.MapPost("/{id:guid}/restore", RestoreTechnology);
    }
    ...
    private static Task<IResult> CreateCategory(NamedRequest request, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Create(request, db, http, true, ct);
    }

    private static Task<IResult> CreateTechnology(NamedRequest request, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Create(request, db, http, false, ct);
    }
    ...
    {
        var errors = InputValidation.Named(request.Name);
        if (errors.Count > 0)
        {
            return ApiProblems.Validation(http, errors);
        }
        ...
        if (category)
        {
            if (await db.SkillCategories.AnyAsync(x => x.NormalizedName == normalized, ct))
            {
                return Conflict(http);
            }

            var value = new SkillCategory { Name = request.Name!.Trim(), NormalizedName = normalized };
            db.SkillCategories.Add(value);
            await db.SaveChangesAsync(ct);
            HttpConcurrency.Set(http.Response, value.Version);
            return Results.Created($"/api/v1/admin/skill-categories/{value.Id}", value.ToResponse());
        }
        if (await db.Technologies.AnyAsync(x => x.NormalizedName == normalized, ct))
        {
            return Conflict(http);
        }

        var technology = new Technology { Name = request.Name!.Trim(), NormalizedName = normalized };
        db.Technologies.Add(technology);
        await db.SaveChangesAsync(ct);
        HttpConcurrency.Set(http.Response, technology.Version);
        return Results.Created($"/api/v1/admin/technologies/{technology.Id}", technology.ToResponse());
    }
    private static Task<IResult> GetCategory(Guid id, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Get(id, db, http, true, ct);
    }

    private static Task<IResult> GetTechnology(Guid id, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Get(id, db, http, false, ct);
    }
    ...
    private static Task<IResult> PatchCategory(Guid id, JsonElement document, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Patch(id, document, db, http, true, ct);
    }

    private static Task<IResult> PatchTechnology(Guid id, JsonElement document, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Patch(id, document, db, http, false, ct);
    }
    ...
    private static async Task<IResult> Patch(Guid id, JsonElement document, PresentationDbContext db, HttpContext http, bool category, CancellationToken ct)
    {
        ...
        var precondition = HttpConcurrency.Validate(http, entity);
        ...
        var current = category ? ((SkillCategory)entity).Name : ((Technology)entity).Name;
        var name = patch.Has("name") ? patch.Read<string>("name") : current;
        var errors = InputValidation.Named(name);
        if (errors.Count > 0)
        {
            return ApiProblems.Validation(http, errors);
        }
        ...
        if (category)
        {
            ((SkillCategory)entity).Name = name!.Trim();
            ((SkillCategory)entity).NormalizedName = normalized;
        }
        else
        {
            ((Technology)entity).Name = name!.Trim();
            ((Technology)entity).NormalizedName = normalized;
        }
        entity.Version++;
    ...
    private static Task<IResult> DeleteCategory(Guid id, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Delete(id, db, http, true, ct);
    }

    private static Task<IResult> DeleteTechnology(Guid id, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Delete(id, db, http, false, ct);
    }
    ...
    private static async Task<IResult> Delete(Guid id, PresentationDbContext db, HttpContext http, bool category, CancellationToken ct)
    {
        ...
        if (referenced)
        {
            return ApiProblems.Create(http, 409, "Resource is in use");
        }
    ...
    private static Task<IResult> RestoreCategory(Guid id, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Restore(id, db, http, true, ct);
    }

    private static Task<IResult> RestoreTechnology(Guid id, PresentationDbContext db, HttpContext http, CancellationToken ct)
    {
        return Restore(id, db, http, false, ct);
    }
    ...
    {
        return ApiProblems.Create(http, 409, "Name already exists");
    }

    private static IResult NotFound(HttpContext http)
    {
        return ApiProblems.Create(http, 404, "Resource not found");
    }
}
